...
**A retrieval system for freight, supply and distribution contracts. Ask a question in
plain language; get the answer back with the exact character span of the clause it came
from.**

Built for the analyst who has to find a liability cap or a termination notice period in a
sixty-page agreement under time pressure. It is a document audit instrument, not a
chatbot: it retrieves and quotes, it does not advise, and every answer is meant to be
checked against the clause beside it.

|  |  |
|---|---|
| **Retrieval** | Dense (pgvector cosine) and lexical (Postgres FTS) fused with RRF, k=60, plus glossary query expansion |
| **Measured at** | Recall@6 **0.690**, Recall@10 **0.770**, MRR **0.624** over 110 questions from 29 real agreements |
| **Latency** | 81 ms p50, of which roughly 95 percent is the query embedding, not the vector search |
| **Guarded by** | A CI gate that fails the build if Recall@6 drops more than two points |
| **Stack** | FastAPI, PostgreSQL 16 + pgvector, SQLAlchemy, Alembic, React 19, Vite, Tailwind v4 |
...
are generated by a script from committed JSON artifacts rather than typed by hand. The
ablation grid below includes the configurations that lost.
...
Two pages, one project, sharing a token layer.

**The landing page** (`/`) is static HTML with an inline motion script. It loads no
framework at all: the scroll-linked scenes, the arcs and the parallax orbs are a few
hundred lines of plain JavaScript, so the marketing surface costs nothing to serve.

**The reader** (`/app.html`) is the product. The contract occupies the left two thirds and
the question sits in a sidebar on the right. An answer arrives as prose with numbered
source chips beneath it; hovering a chip lights its clause in the document, clicking one
scrolls to it. Citation highlights are placed from the character offsets the API returns,
so a highlight cannot drift from the span it claims -- a property the smoke test asserts
on every run.

When no generation provider is configured, answers are built by
`chainlens.generation.extractive`: query-focused sentence selection over the retrieved
clauses, with redaction notices and page furniture filtered out and repetition suppressed.
It quotes; it cannot invent. Set `CHAINLENS_GOOGLE_API_KEY` to use Gemini instead.
...
cp .env.example .env          # then set CHAINLENS_DATABASE_URL to that container
pip install -e ".[dev]"
```

```bash
# terminal 1: migrate, index the corpus if empty, and serve, in one process
...
The landing page is then at `http://localhost:5173/` and the reader at `/app.html`.
`scripts/serve.py` is idempotent: it migrates to head and indexes the 29-contract corpus
only if the database is empty, so the first run takes about thirty seconds longer than
every run after it.

Without Docker, `CHAINLENS_LOCAL_PG=1` starts an embedded PostgreSQL instead. Full
step-by-step instructions, including the failure modes, are in [`RUNNING.md`](RUNNING.md).
...
Stated plainly, because these are the questions a reader should ask.

- **It does not advise.** It locates and quotes contract language. Interpreting that
  language is the reader's job.
- **The embedding model is a substitute.** `lsa-tfidf-svd-384` is a corpus-fitted LSA
  model, used because the model hub was unreachable in the environment these numbers were
  produced in. The absolute scores are a floor for the architecture, not a claim about a
  modern encoder. See ADR-0003.
- **Cross-encoder reranking is implemented but unmeasured.** The weights could not be
  downloaded, so those cells render as a dash with the reason, never as a number.
- **Extraction is pattern-based, not model-based.** That is a real compromise, argued with
  its numbers in `docs/EXTRACTION.md`.
